...
const Add_Address = (props) => {
    ...
    const onNewAddress = async () => {
        ...
        if (validaName(name) && validaPhone(phone) && validaAddress(address)) {
            console.log(result);
            try {
                const response = await AxiosIntance().post("/api/addresses/add-new-address", { receiver_name: name, address: address, phone: phone, user_id: user._id });
        ...
        <View style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <View style={{ paddingHorizontal: 16 }}>
                    ...
                    <TextInput style={styles.input} onChangeText={(address) => setAddress(address)} />
                    {
                    ...
                    <TextInput style={styles.input} onChangeText={(phone) => setPhone(phone)} keyboardType='numeric' />
                    {
